scenarios.push({
    ...
    logs: {
        ...
        email: [
            ...
            "2026-05-29T11:00:00Z - Internal - From: helpdesk-admin@example.com - To: bjohnson@example.com - Subject: I've fixed the issue on WKSTN-05, let me know if it recurs. - Status: Sent"
        ],
        file_server: [
    ...
    principles: ["Lateral Movement", "Unauthorized Privilege Use", "Persistence", "Phishing / Social Engineering"],
    explanation: "This incident is a classic 'Helpdesk Trap'. User bjohnson on WKSTN-05 was initially compromised via a drive-by download from a lookalike support portal. When the helpdesk administrator logged in interactively (Logon Type 2) to troubleshoot the 'weird' behavior reported by the user, the resident malware (running as SYSTEM) accessed the LSASS process (Sysmon Event ID 10) to dump the administrator's credentials. The attacker then used these high-privilege credentials to move laterally to the database servers using PsExec, as evidenced by the creation of the PSEXESVC service and subsequent network logons. Benign activity from asmith and fsmith serves as the background noise."
});
